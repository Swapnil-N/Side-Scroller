...
public class Game extends JPanel implements KeyListener, Runnable
{
	...
	public Game()
	{
		...
		try {
			spriteSheet = ImageIO.read(new File("res/d"));
			for(int x=0;x<marioBuffImg.length;x++)
				marioBuffImg[x]=spriteSheet.getSubimage(x*55+5,200,55,50);
		...
		for(int x=0;x<marioImgs.length;x++)
			marioImgs[x]=marioBuffImg[x].getScaledInstance(120, 120, Image.SCALE_DEFAULT);
