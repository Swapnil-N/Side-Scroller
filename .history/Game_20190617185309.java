...
public class Game extends JPanel implements KeyListener, Runnable
{
	...
	BufferedImage[] heroBuffImg = new BufferedImage[12];
	Image[] heroImgs = new Image[12];
	...
	public Game()
	{
		...
		try {
			...
			for(int x=0;x<heroBuffImg.length;x++)
				heroBuffImg[x]=spriteSheet.getSubimage(x*55+7,202,55,50);
		...
		for(int x=0;x<heroImgs.length;x++)
			heroImgs[x]=heroBuffImg[x].getScaledInstance(120, 120, Image.SCALE_DEFAULT);
		...
		while(true){
			if(gameOn){
				...
				if (right){
					...
					backX-=3;
					imgCount++;
					if(imgCount>heroBuffImg.length)
						imgCount=6;
				}
				if (left){
					//x-=3;
	...
	public void paintComponent(Graphics g){
		...
		g2d.drawImage(heroImgs[imgCount],100,heroY,null);
